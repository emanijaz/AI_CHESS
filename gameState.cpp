...
void gameState::promotePawn(int pawnCode, action& Move){
    if(pawnCode == 1){
        if(Move.toRow == 7 - Board.whiteHome){
            Board.board[Move.toRow][Move.toCol] = 5;
            computePossibleActions();
            if(Actions.getActionCount() > 0)
                return ;

            Board.board[Move.toRow][Move.toCol] = 4;
            computePossibleActions();
            if(Actions.getActionCount() > 0)
                return ;

            Board.board[Move.toRow][Move.toCol] = 3;
            computePossibleActions();
            if(Actions.getActionCount() > 0)
                return ;

            Board.board[Move.toRow][Move.toCol] = 2;
        }
    }
    else{
        if(Move.toRow == Board.whiteHome){
            Board.board[Move.toRow][Move.toCol] = -5;
            computePossibleActions();
            if(Actions.getActionCount() > 0)
                return ;
...
int gameState::applyMove(action& Move){
    ...
    if(Board.board[Move.fromRow][Move.fromCol]){
